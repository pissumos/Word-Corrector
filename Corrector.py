    ...
    for i in range(len(word)):
        if i is not (len(word) - 1):  # insertion done
            ...
            if dictionary_words.__contains__(w):
                edit_words.append(w)
                edit_ratios.append(float(insertion_matrix[first][second]) / insertion_total *
            ...
            if dictionary_words.__contains__(w):
                edit_words.append(w)
                edit_ratios.append(float(trans_matrix[first][second]) / trans_total *
                                   float(dictionary_word_counts[dictionary_words.index(w)]) / total_count)
        for letter in alphabet:
            if letter is not word[i]:  # subs done
                ...
                if dictionary_words.__contains__(w):
                    edit_words.append(w)
                    edit_ratios.append(float(subs_matrix[first][second]) / subs_total *
                                       float(dictionary_word_counts[dictionary_words.index(w)]) / total_count)
            if i == len(word) - 1:
                w = word + letter
                first = asc.checkAscii(ord(word[i])) - 97
                ...
                if dictionary_words.__contains__(w):
                    ...
                    edit_ratios.append(float(deletion_matrix[first][second]) / deletion_total *
                                       float(dictionary_word_counts[dictionary_words.index(w)]) / total_count)
            else:
